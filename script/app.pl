...
use Shop::Func::HKT;
use Shop::Func::Codensity;
use Shop::Func::Validation;
use Shop::Func::Reader;
use Shop::Func::State;
use Shop::Func::Writer;
...
handle {
    ...
    my $eod_prices = Shop::Func::HKT::fmap2(
        $all_products,
    ...
    my $total_value = Shop::Func::HKT::fold_sum($prod_prices);
    my $n_premium   = Shop::Func::HKT::fold_count($prod_prices, sub ($p) { $p >= 3000 });
    my $has_budget  = Shop::Func::HKT::fold_any($prod_prices, sub ($p) { $p < 2000 });
    my $all_stocked = Shop::Func::HKT::fold_all($all_products, sub ($p) { $p->stock > 0 });
    ...
    my $catalog = Shop::Func::HKT::map_reduce(
        $all_products,
    ...
    my $top_item  = Shop::Func::HKT::head_option($expensive);
    my $cheap_hit = Shop::Func::HKT::head_option($cheap);
    Shop::Infra::Display::kv("head_option(expensive)", Shop::Func::HKT::show_option(
        Shop::Func::HKT::option_fmap($top_item, sub ($p) { $p->name })));
    Shop::Infra::Display::kv("head_option(cheap)", Shop::Func::HKT::show_option($cheap_hit));

    # Option monad: chain nullable stock check
    my $stock_msg = Shop::Func::HKT::option_bind($top_item, sub ($p) {
        $p->stock > 0 ? Some($p->name . ": " . $p->stock . " left") : None();
    });
    Shop::Infra::Display::kv("option_bind(stock)", Shop::Func::HKT::show_option($stock_msg));

    # Natural transformation round-trip: ArrayRef ~> Option ~> ArrayRef
    my $roundtrip = Shop::Func::HKT::option_to_list(
        Shop::Func::HKT::option_fmap($top_item, sub ($p) { $p->name }),
    );
    Shop::Infra::Display::kv("option_to_list . head", "[" . join(", ", @$roundtrip) . "]");

    # Kleisli composition: find -> classify pipeline
    my $find_and_classify = Shop::Func::HKT::kleisli(
        sub ($pid) {
            my $opt = Shop::Domain::Inventory::find_product($pid);
            Shop::Func::HKT::option_to_list(
                Shop::Func::HKT::option_fmap($opt, sub ($p) { $p }),
            );
        },
    ...
    my $products_c = Shop::Func::Codensity::lift_list($all_products);
    my $quarters   = Shop::Func::Codensity::lift_list([qw(Q1 Q2 Q3 Q4)]);

    my $projection = Shop::Func::Codensity::bind($products_c, sub ($p) {
        Shop::Func::Codensity::bind($quarters, sub ($q) {
            Shop::Func::Codensity::unit($p->name . "/" . $q);
        });
    });
    ...
    my $flat_audit = Shop::Func::HKT::mjoin($nested_audit);
    ...
    my $v_name  = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_name("Widget Pro"));
    my $v_price = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_price(2500));
    my $v_qty   = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_quantity(10));

    my $v_product = Shop::Func::Validation::validation_lift_a3(
        sub ($name, $price, $qty) { "$name: \$$price x$qty" },
        $v_name, $v_price, $v_qty,
    );
    Shop::Infra::Display::kv("Valid product", Shop::Func::Validation::show_validation($v_product));

    # Now with multiple errors
    my $v_bad_name  = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_name(""));
    my $v_bad_price = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_price(-100));
    my $v_bad_qty   = Shop::Func::Validation::result_to_validation(Shop::Feature::Analytics::validate_quantity(-5));

    my $v_bad_product = Shop::Func::Validation::validation_lift_a3(
        sub ($name, $price, $qty) { "$name: \$$price x$qty" },
        $v_bad_name, $v_bad_price, $v_bad_qty,
    );
    Shop::Infra::Display::kv("Invalid product", Shop::Func::Validation::show_validation($v_bad_product));

    # Batch validation
    my $prices = [100, -50, 200, 0, 300];
    my $v_batch = Shop::Func::Validation::validate_all($prices, sub ($p) {
        $p > 0 ? Valid($p) : Invalid(["Invalid price: $p"]);
    });
    Shop::Infra::Display::kv("Batch validate prices", Shop::Func::Validation::show_validation(
        Shop::Func::Validation::validation_fmap($v_batch, sub ($arr) { "[" . join(", ", @$arr) . "]" }),
    ));

    # Validation <-> Result conversion
    my $back_to_result = Shop::Func::Validation::validation_to_result($v_bad_product);
    Shop::Infra::Display::kv("validation_to_result", Shop::Func::HKT::show_result($back_to_result));
    ...
    my $config = Shop::Func::Reader::ShopConfig(
        tax_rate                => 10,
    ...
    # Simple readers
    my $widget_taxed = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::price_with_tax(1500), $config,
    );
    Shop::Infra::Display::kv("Widget + tax(10%)", "\$$widget_taxed");

    my $ship_small = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::shipping_cost(3000), $config,
    );
    my $ship_large = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::shipping_cost(6000), $config,
    );
    ...
    my $order_total = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::order_total_with_tax_and_shipping(7700), $config,
    );
    Shop::Infra::Display::kv("Order total (tax + shipping)", "\$$order_total");

    # local: temporarily override config
    my $high_tax_total = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::local(
            sub ($cfg) { $cfg->with(tax_rate => 20) },
            Shop::Func::Reader::price_with_tax(1500),
        ),
        $config,
    );
    ...
    # format_price
    my $formatted = Shop::Func::Reader::run_reader(
        Shop::Func::Reader::format_price(7700), $config,
    );
    ...
    my $cart_computation = Shop::Func::State::state_bind(
        Shop::Func::State::add_to_cart(
            OrderItem(product_id => ProductId("WIDGET"), quantity => 3, unit_price => 1500),
        ),
        sub ($) {
            Shop::Func::State::state_bind(
                Shop::Func::State::add_to_cart(
                ...
                sub ($) {
                    Shop::Func::State::state_bind(
                        ...
                        sub ($) { Shop::Func::State::cart_summary() },
                    );
                },
            );
        },
    ...
    my $summary_text = Shop::Func::State::eval_state($cart_computation, Shop::Func::State::empty_cart());
    ...
    my $audit = Shop::Func::Writer::writer_bind(
        Shop::Func::Writer::subtotal_with_audit($alice_items),
        sub ($subtotal) {
            Shop::Func::Writer::writer_bind(
                Shop::Func::Writer::discount_with_audit($subtotal, $alice_disc),
                sub ($final) {
                    Shop::Func::Writer::writer_bind(
                        Shop::Func::Writer::tell("  Final price: \$$final"),
                        sub ($) { Shop::Func::Writer::writer_pure($final) },
                    );
                },
            );
        },
    ...
    my $all_ok = Shop::Func::HKT::sequence_result([Ok(1), Ok(2), Ok(3)]);
    Shop::Infra::Display::kv("sequence [Ok 1..3]", Shop::Func::HKT::show_result(
        Shop::Func::HKT::result_fmap($all_ok, sub ($arr) { "[" . join(", ", @$arr) . "]" }),
    ));

    my $has_err = Shop::Func::HKT::sequence_result([Ok(1), Err("boom"), Ok(3)]);
    Shop::Infra::Display::kv("sequence [Ok,Err,Ok]", Shop::Func::HKT::show_result($has_err));

    # traverse_result: validate + collect
    my $valid_ids = Shop::Func::HKT::traverse_result(
    ...
    Shop::Infra::Display::kv("traverse products", Shop::Func::HKT::show_result(
        Shop::Func::HKT::result_fmap($valid_ids, sub ($arr) { "[" . join(", ", @$arr) . "]" }),
    ));

    # traverse with a missing product
    my $with_missing = Shop::Func::HKT::traverse_result(
    ...
    Shop::Infra::Display::kv("traverse w/ missing", Shop::Func::HKT::show_result($with_missing));

    # sequence_option
    my $all_some = Shop::Func::HKT::sequence_option([Some(10), Some(20), Some(30)]);
    Shop::Infra::Display::kv("sequence [Some 10..30]", Shop::Func::HKT::show_option(
        Shop::Func::HKT::option_fmap($all_some, sub ($arr) { "[" . join(", ", @$arr) . "]" }),
    ));

    my $has_none = Shop::Func::HKT::sequence_option([Some(10), None(), Some(30)]);
    Shop::Infra::Display::kv("sequence [Some,None,Some]", Shop::Func::HKT::show_option($has_none));

    # lift_a2_result: combine two results
    my $combined = Shop::Func::HKT::lift_a2_result(
        sub ($a, $b) { $a + $b },
        Ok(100), Ok(200),
    );
    Shop::Infra::Display::kv("lift_a2(+, Ok 100, Ok 200)", Shop::Func::HKT::show_result($combined));

    my $combined_err = Shop::Func::HKT::lift_a2_result(
        sub ($a, $b) { $a + $b },
        Err("no A"), Ok(200),
    );
    Shop::Infra::Display::kv("lift_a2(+, Err, Ok 200)", Shop::Func::HKT::show_result($combined_err));
